...
# 4/26 HACK: upload RFC 1 to 999 without leading zeros,
# make new uploads to turn old DOIs to stubs,
# make text file of aliases
...
class DOIsub:
    ...
    def addlink(self, rfc):
        """
        add link to new DOI
        remove everything else
        """
        rp = ET.SubElement(self.body, "report-paper")
        rp.append(ET.Comment("Stub for {0} {1}".format(rfc['doc-id'], rfc['stream'])))
        ...
        olddocid = rfc['doc-id']
        r = re.match(r'RFC0*(\d+)', rfc['doc-id'])
        assert r
        docid = f"RFC{r.group(1)}"
        print("docid", docid)
        ET.SubElement(pi, "item_number").text = docid
        # add link
        ET.SubElement(pi, "identifier", id_type="doi").text = f"10.17487/{docid}"

        # assign the doi, and the redirect URL
        # no timestamp, defaults to the one in the head
        dd = ET.SubElement(rpm, "doi_data")
        # old DOI, new link
        ET.SubElement(dd, "doi").text = f"10.17487/{olddocid}"
        ET.SubElement(dd, "resource").text = params['doiurl'].format(docid).lower()
        self.aliases += f"10.17487/{docid}\t10.17487/{olddocid}\n"
...
#        docid = f"RFC{r.group(1)}"
        docid = rfc['doc-id']
        print("docid", docid)
        ET.SubElement(pi, "item_number").text = olddocid

        # assign the doi, and the redirect URL
        # no timestamp, defaults to the one in the head
        dd = ET.SubElement(rpm, "doi_data")
        ET.SubElement(dd, "doi").text = f"10.17487/{docid}"
        ET.SubElement(dd, "resource").text = params['doiurl'].format(docid).lower()


    def alias(self, filename):
        """
        write an alias file for crossref
        """
        with open(filename, "w") as of:
            of.write(self.aliases)
...
### 10/21 does not work, Crossref returns empty answers
    ...
    def findnew(self, rfcs, dois, maxrfc=100, redo=False, link=False):
        """
        ...
        for r in rfcs.keys():
            rl = r.lower()
            if not rl.startswith('rfc0'):   # ignore all past rfc0999
                continue
            print("try", rl, maxrfc)
            if rl not in self.rfcs or (redo and self.rfcs[rl][0] != 'S'):
                if link:
                    dois.addlink(rfcs.get(r))
                else:
...
#    parser.add_argument("--resync", action='store_true', help="Resync with Crossref database")
    ...
    parser.add_argument("--link", action='store_true', help="make link entries old->new")
    parser.add_argument("--alias", type=str, help="Output alias file")
    ...
    if args.batch or args.write or args.upload:
        if args.batch:
            rfclist = dstatus.findnew(rfcs, dois, maxrfc=args.batch, link=args.link)
        else:
            rfclist = dstatus.findnew(rfcs, dois, link=args.link)
        if rfclist:
    ...
    if args.alias and rfclist:
        dois.alias(args.alias)
